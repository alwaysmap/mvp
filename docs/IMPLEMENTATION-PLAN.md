...
## Phase 3: User Workflow Integration (Current Phase)

**Goal:** Complete the end-to-end user workflow from map creation to PNG export.

### Background

Phases 0-2 have been completed:
- ✅ Layout engine and rendering (Phase 1)
- ✅ Database schema and queue infrastructure (Phase 2)
- ✅ Export pipeline with Puppeteer (Phase 2)
- ✅ 260 integration tests + 42 E2E tests passing

**Current state:**
- Map Editor (`/create-map`) provides interactive preview with all controls
- Backend API endpoints exist for export and job management
- pg-boss worker processes export jobs asynchronously
- PNG generation works at 300 DPI with sRGB profiles

**Gap:**
- No way to save map configuration from UI
- No "Buy" button to trigger export workflow
- No job status polling for user feedback

**See `docs/NEXT-STEPS.md` for detailed implementation tasks.**

### Timeline: 6 days

| Day | Task | Deliverable |
|-----|------|-------------|
| 1 | Save Map | POST /api/maps + UI |
| 2 | Print Options | UI panel for specs |
| 3 | Buy Button | Trigger export workflow |
| 4 | Job Status | Polling UI component |
| 5 | Worker Verification | docker-compose worker running |
| 6 | E2E Testing | Complete workflow test |

### Definition of Done

Phase 3 is complete when:
1. User can save map configuration from UI
2. User can select page size and orientation
3. User can click "Buy" to trigger export
4. User sees job status and PNG auto-downloads
5. All existing tests passing (260 integration + 42 E2E)
6. New E2E tests passing (≥5 new tests for Phase 3 features)
7. docker-compose up brings up full working system

---

**Last Updated:** 2025-12-07 (Phase 3 planning complete)
